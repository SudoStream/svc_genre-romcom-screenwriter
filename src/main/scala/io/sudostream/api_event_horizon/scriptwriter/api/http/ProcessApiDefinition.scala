...
trait ProcessApiDefinition extends Health
  ...
  private def processFile(fileData: Multipart.FormData): Future[GeneratedTestsEvent] = {
    ...
    swaggerDefinitionFuture flatMap {
      swaggerDefinition => (swaggerSpecActor ? swaggerDefinition).asInstanceOf[Future[GeneratedTestsEvent]]
    }
  }
